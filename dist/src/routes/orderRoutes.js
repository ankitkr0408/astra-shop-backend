...
router.route('/:id/pay').put(authMiddleware_1.protect, orderController_1.updateOrderToPaid); // Using PUT for updates
router.route('/:id/deliver').put(authMiddleware_1.protect, authMiddleware_1.admin, orderController_1.updateOrderToDelivered);
router.route('/payment/verify').post(authMiddleware_1.protect, orderController_1.verifyPayment);
